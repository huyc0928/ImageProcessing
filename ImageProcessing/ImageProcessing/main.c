...
void task1func1(unsigned char* image, unsigned char* output, int x, int y, double originx, double originy, int width);
double distance(double x1, double y1, double x2, double y2);
int main()
{
	int width, height, channels;
	unsigned char* image = stbi_load("a.jpg", &width, &height, &channels, 1);//读入图像用的函数
	...
	stbi_write_jpg("ex1.jpg", width, height, 1, output, 100);//写出函数的图像
	stbi_image_free(image);//释放内存的函数
	return 0;
}
void task1(unsigned char* image, unsigned char* output, int width, int height, int channels) {
	...
	for (int x = 0; x < width; x++) {//x对应横坐标
		for (int y = 0; y < height; y++) {//y对应纵坐标
			...
			switch (ctrl) {
			case 1://最近邻插值法
				task1func1(image, output, x, y, originx, originy, width);
